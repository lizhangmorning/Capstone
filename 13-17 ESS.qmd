...
for (k in seq(0, 1, by = 0.002)) {
  ...
  # C1, C2: 每个成分的边际似然乘以似然（比例项）
  C1 <- compute_Cj_exact(aj1, bj1, ped_drug_r, ped_drug_n)
  C2 <- compute_Cj_exact(aj2, bj2, ped_drug_r, ped_drug_n)
  Cj <- c(C1, C2)
  ...
  alpha_prior_placebo <- k * alpha_adult_placebo + (1 - k) * alpha_skeptical
  beta_prior_placebo <- k * beta_adult_placebo + (1 - k) * beta_skeptical
  alpha_prior_drug <- k * alpha_adult_drug + (1 - k) * alpha_skeptical
  beta_prior_drug <- k * beta_adult_drug + (1 - k) * beta_skeptical
  ...
  # 从混合后的分布中进行采样
  samples_adult_placebo <- rbeta(n_samples, alpha_post_adult_placebo, beta_post_adult_placebo)
  samples_adult_drug <- rbeta(n_samples, alpha_post_adult_drug, beta_post_adult_drug)

  samples_skeptical_placebo <- rbeta(n_samples, alpha_post_skeptical_placebo, beta_post_skeptical_placebo)
  samples_skeptical_drug <- rbeta(n_samples, alpha_post_skeptical_drug, beta_post_skeptical_drug)

  # 按权重混合采样
  samples_placebo <- k * samples_adult_placebo + (1 - k) * samples_skeptical_placebo
  samples_drug <- k1[1] * samples_adult_drug + k1[2] * samples_skeptical_drug
  ...
  # ESS 计算
  var_post_placebo <- var(samples_placebo)
  var_post_drug <- var(samples_drug)
...
# 输出结果
cat("✅ Tipping Point Found:\n")
cat("Adult Prior Weight (k):", tipping_point$k, "\n")
...
p1 <- ggplot(results, aes(x = k)) +
  ...
  geom_vline(xintercept = tipping_point$k, linetype = "dashed", color = "darkred") +
  labs(
    title = "Posterior Response Rate Difference (Drug - Placebo)",
    x = "Adult Prior Weight (k)",
...
p2 <- ggplot(results, aes(x = k)) +
  ...
  geom_vline(xintercept = tipping_point$k, linetype = "dotted", color = "darkgreen") +
  labs(
