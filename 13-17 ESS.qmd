...
# 遍历 adult 信息的权重 a ∈ [0, 1]
for (a in seq(0, 1, by = 0.002)) {
  # 形成混合先验参数（adult 与 skeptical）
  ...
  alpha_post_adult_placebo <- alpha_adult_placebo + ped_placebo_r
  beta_post_adult_placebo <- beta_adult_placebo + ped_placebo_n -ped_placebo_r
  alpha_post_adult_drug <- alpha_adult_drug + ped_drug_r
  beta_post_adult_drug <- beta_adult_drug + ped_drug_n - ped_drug_r

# 后验B: 怀疑先验 + 儿童数据
  alpha_post_skeptical_placebo <- alpha_skeptical + ped_placebo_r
  beta_post_skeptical_placebo <- beta_skeptical + ped_placebo_n - ped_placebo_r
  alpha_post_skeptical_drug <- alpha_skeptical + ped_drug_r
  beta_post_skeptical_drug <- beta_skeptical + ped_drug_n - ped_drug_r

  set.seed(123)

# Step 1: 计算混合后的参数
alpha_mixed_placebo <- a * alpha_post_adult_placebo + (1 - a) * alpha_post_skeptical_placebo
beta_mixed_placebo <- a * beta_post_adult_placebo + (1 - a) * beta_post_skeptical_placebo

alpha_mixed_drug <- a * alpha_post_adult_drug + (1 - a) * alpha_post_skeptical_drug
beta_mixed_drug <- a * beta_post_adult_drug + (1 - a) * beta_post_skeptical_drug

# Step 2: 从混合后的分布中进行采样
samples_placebo <- rbeta(n_samples, alpha_mixed_placebo, beta_mixed_placebo)
samples_drug <- rbeta(n_samples, alpha_mixed_drug, beta_mixed_drug)
  ...
  prob_superior <- mean(diff > 0)  # 药物优效的概率

  # ESS 计算（方差缩减法）
var_post_placebo <- (alpha_mixed_placebo * beta_mixed_placebo) /
  ((alpha_mixed_placebo + beta_mixed_placebo)^2 * (alpha_mixed_placebo + beta_mixed_placebo + 1))

var_post_drug <- (alpha_mixed_drug * beta_mixed_drug) /
  ((alpha_mixed_drug + beta_mixed_drug)^2 * (alpha_mixed_drug + beta_mixed_drug + 1))

# ESS 计算（基于方差缩减）
ess_placebo <- max(0, ped_placebo_n * (var_placebo_no_prior / var_post_placebo - 1))
ess_drug <- max(0, ped_drug_n * (var_drug_no_prior / var_post_drug - 1))
...
p2 <- ggplot(results, aes(x = a)) +
  geom_line(aes(y = prob_superior), color = "purple", size = 1.2) +
  geom_hline(yintercept = 0.975, linetype = "dashed", color = "red") +
  geom_vline(xintercept = tipping_point$a, linetype = "dotted", color = "darkgreen") +
  labs(
    title = "Pr(Drug > Placebo) vs Adult Prior Weight",
    x = "Adult Prior Weight (a)", y = "Pr(Drug > Placebo)"
  ) + theme_minimal()

p3 <- ggplot(results, aes(x = a)) +
  geom_line(aes(y = ESS_total), color = "darkorange", size = 1.2) +
