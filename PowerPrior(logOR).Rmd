...
logor_samples <- samples_matrix[, "log_odds_ratio"]
median_logor <- median(logor_samples)
ci_95_logor <- quantile(logor_samples, c(0.025, 0.975))
prob_gt_0 <- mean(logor_samples > 0)  # Probability that log(OR) > 0

  
  return(list(
    median_logor = median_logor,
    ci_95 = ci_95_logor,
    prob_gt_0 = prob_gt_0,
    samples = logor_samples
  ))
}
...
```{r}
#------ Method 2: Direct Beta-Binomial Approach with log(OR) ------
  ...
  for (alpha in alpha_seq) {
    if (method %in% c("both", "RJAGS")) {
      ...
      results <- rbind(results, data.frame(
        alpha = alpha,
        method = "RJAGS",
        median_logor = rjags_res$median_logor,
    ...
    if (method %in% c("both", "Beta-Binomial")) {
      ...
      results <- rbind(results, data.frame(
        ...
        median_logor = beta_res$median_logor,
